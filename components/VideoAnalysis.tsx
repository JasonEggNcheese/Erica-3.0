...
import { UploadCloud, Film, Wand2, XCircle, Loader2 } from 'lucide-react';

const VideoAnalysis: React.FC = () => {
  const { videoFile, setVideoFile, analysis, isLoading, error, progress, progressMessage, analyzeVideo, setAnalysis, setError } = useVideoAnalysis();
  ...
  const handleAnalyze = () => {
    analyzeVideo("Provide a concise summary of the events in this video.");
  };
  ...
  return (
    <div className="p-4 md:p-8 h-full flex flex-col gap-6 text-white overflow-y-auto">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 h-full">
        {/* Left Column: Uploader and Video Preview */}
          ...
          {videoSrc ? (
            <div className="relative aspect-video rounded-lg overflow-hidden border-2 border-gray-700">
                ...
                <button 
                  ...
                  className="absolute top-2 right-2 bg-black/50 hover:bg-red-600/80 rounded-full p-1.5 transition-colors"
        ...
        <div className="flex flex-col gap-4">
           <h2 className="text-xl font-semibold text-purple-300">2. Analysis Result</h2>
            {isLoading && (
